...
workflow Pbmm2AlignmentWithMetrics {
    ...
    call UnzipReferenceIfCompressed as UnzipHap1 {  
        input:  
            reference_fasta = reference_fasta_hap1  
    }  

    call UnzipReferenceIfCompressed as UnzipHap2 {  
        input:  
    ...
    call PB.Align as AlignHap1 {
        input:
            bam         = ccs_fq,
            ref_fasta   = UnzipHap1.unzipped_fasta,
    ...
    call PB.Align as AlignHap2 {
        input:
            bam         = ccs_fq,
            ref_fasta   = UnzipHap2.unzipped_fasta,
            sample_name = sample_name,
            library     = lib_name,
            map_preset  = preset,
            drop_per_base_N_pulse_tags = true
...
task UnzipReferenceIfCompressed {  
    input {  
    ...
    command <<<  
        ...
        if [[ ~{reference_fasta} == *.gz ]]; then  
            gunzip -c ~{reference_fasta} > uncompressed_reference.fasta  
        else  
            cp ~{reference_fasta} uncompressed_reference.fasta  
        fi  
    >>>  

    output {  
        File unzipped_fasta = "uncompressed_reference.fasta"  
    }  
